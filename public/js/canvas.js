...
function Component(x, y, size, settings){
    ...
    this.getWidth = function() {
        if(this.group.scaleY != undefined){//Component is in a group
            return this.group.getXScale() * this.size.width;
        }
        return this.size.width;
    }
    this.getHeight = function() {
        if(this.group.scaleY != undefined){//Component is in a group
            return this.group.getYScale() * this.size.height;
        }
        return this.size.height;
    }
    this.getOpacity = function() {
        if(this.group.opacity != undefined){//Component is in a group
            return this.group.getOpacity() * this.opacity;
        }
        return this.opacity;
    }
        ...
        case 'text':
            this.draw = function() {
                if(this.isShowing()){
                    ...
                    this.canvas.ctx.globalAlpha = this.getOpacity();
                    var textSize = this.size.font * this.canvas.scaled;

                    this.canvas.ctx.font = [this.modifiers, textSize+"px", this.font].join(" ");

                    this.canvas.ctx.fillStyle = this.fill.color;

                    var i;
                    for(i=0;i<this.text.length;i++){
                        this.canvas.ctx.fillText(this.text[i], this.getX()*this.canvas.scaled, (this.getY()*this.canvas.scaled)+textSize*i);
                    } 
                }
            }
        break;
        ...
        case 'image':
            this.draw = function() {
                if(this.isShowing()){
                    ...
                    */
                    this.canvas.ctx.globalAlpha = this.getOpacity();
                    this.canvas.ctx.drawImage(this.source, this.size.sx || 0, this.size.sy || 0, this.size.sWidth || this.source.naturalWidth, this.size.sHeight || this.source.naturalHeight, this.getX()*this.canvas.scaled, this.getY()*this.canvas.scaled, this.getWidth()*this.canvas.scaled, this.getHeight()*this.canvas.scaled);
        ...
        default:
            this.draw = function() {//Rectangle
                if(this.isShowing()){
                    ...
                    */
                    var v = {x:this.getX()*this.canvas.scaled, y:this.getY()*this.canvas.scaled, w:this.getWidth()*this.canvas.scaled, h:this.getHeight()*this.canvas.scaled,};
                    this.canvas.ctx.globalAlpha = this.getOpacity();

                    this.canvas.ctx.shadowColor = this.shadow.color;
                    this.canvas.ctx.shadowBlur = this.shadow.intensity;
...
function Group(x, y, scaleX, scaleY, group, canvas){
    ...
    this.getOpacity = function() {
        if(this.group.opacity != undefined){//Group is in a group
            return this.group.getOpacity() * this.opacity;
        }
        return this.opacity;
    }
...
function changeFrameRate(newFrameRate) {
    ...
    updateInterval = setInterval(onFrame, 1000 / (frameRate*60));
  }
...
function drawComps(){
    ...
    for(i=0;i<allComps.length;i++){//For each canvas
        ...
        for(var j=0;j<allComps[i].length;j++){
            var smallestComp = allComps[i][j];
            var compI = j;
            for(var k=j+1;k<allComps[i].length;k++){
                if(allComps[i][k].z < smallestComp.z){
                    smallestComp = allComps[i][k];
                    compI = k;
                }
            }
            if(compI != j){
                allComps[i].splice(compI, 1);
                allComps[i].splice(j, 0, smallestComp);
            }
            allComps[i][j].draw();
        }
        /*
        for(var j=0;j<allComps[i].length;j++){//For each component in a canvas
            allComps[i][j].draw();
        }
        */
    }
